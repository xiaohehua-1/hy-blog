...
.volume-slider-wrapper {
  ...
  padding: 10px 0; justify-content: center; z-index: 10;
  /* 使用 opacity 和 visibility 控制显示隐藏，并添加延迟 */
  opacity: 0;
  visibility: hidden;
  transition: opacity 0.2s ease-in-out 0.1s, visibility 0.2s ease-in-out 0.1s;
  display: flex; /* 默认保持flex布局，通过visibility控制可见性 */
}
.volume-box:hover .volume-slider-wrapper {
  opacity: 1;
  visibility: visible;
  transition-delay: 0s; /* 鼠标进入时立即显示 */
}
...
/* 修复点：动态线条样式
  ...
  2. 确保高度固定
*/
.dynamic-icon { display: flex; align-items: flex-end; gap: 3px; height: 18px; overflow: hidden; }
.bar {
  width: 3px; 
